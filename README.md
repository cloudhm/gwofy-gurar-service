...
| POST | `/api/install` | **Session JWT**（与 `/api/me` 相同）。用 App Bridge session token 做 [token exchange](https://shopify.dev/docs/apps/build/authentication-authorization/access-tokens/token-exchange) 换取 **expiring offline** 对并写入 `METADATA`（逻辑对齐 `/oauth/callback`）。首次安装或 `installation_status` 非 **ACTIVE** / 无 `access_token_enc` 时异步入队 **`INITIAL_SYNC`**（`bootstrap_enqueued: true`）；已安装且 **ACTIVE** 时仅刷新 token（`bootstrap_enqueued: false`）。**401** `session_token_exchange_failed`；**403** `plugin_suspended`。嵌入式 App 在调用 **`POST /api/activate`** 前应先调本接口，避免 **`shop_not_installed`**。 |
